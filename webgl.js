!(function(e) {
  setTimeout(() => {
    ...
    container.parentElement.addEventListener('mousemove', (e) => {
      ...
      if (
        !$(e.target).is('a') &&
        $(e.target).closest('.text-corner').length == 0 &&
        $(e.target).closest('.text-center').length == 0
      ) {
        pointers[0].down = true

        pointers[0].color = [1 + 0.2, 1 + 0.2, 1 + 0.2]
    ...
    container.parentElement.addEventListener('touchstart', (e) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        pointers[i].color = [1 + 0.2, 1 + 0.2, 1 + 0.2]
      }
    })
    ...
    function getWebGLContext(canvas) {
      ...
      // gl.setScale(0.5, 0.5);
      // console.log(canvas.getContext('webgl2', params))
